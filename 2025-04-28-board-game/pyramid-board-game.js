...
function createPoints() {
  ...
  points[0].x = canvasW * .1
  points[0].y = canvasH * .9
  
  points[points.length -1].x = canvasW * .9
  points[points.length -1].y =  canvasH * .1
}
...
function movePoints() {
  ...
  for (let i = 1; i < points.length - 1; i++) {
    ...
    points[i].vx = constrain(points[i].vx * drag,-maxSpeed, maxSpeed)
    points[i].vy = constrain(points[i].vy * drag,-maxSpeed, maxSpeed)
